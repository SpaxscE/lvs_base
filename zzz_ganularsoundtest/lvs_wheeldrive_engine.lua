...
function ENT:OnEngineActiveChanged( Active )
	...
	for id, data in pairs( self.EngineSounds ) do
		...
		if data.SoundType == LVS.SOUNDTYPE_GRANULAR then

			self.EngineSounds[ id ].SoundFile = data.sound
			self.EngineSounds[ id ].SoundType = data.SoundType
			self.EngineSounds[ id ].NextUpdate = 0
...
function ENT:HandleEngineSounds( vehicle )
	...
	for id, data in pairs( EntTable.EngineSounds ) do
		if data.SoundType ~= LVS.SOUNDTYPE_GRANULAR then continue end
		if data.NextUpdate > T then continue end

		local Vol03 = 0.3
		local Vol02 = 0.2

		local PitchAdd = (CurrentGear - 1) * (1 / (NumGears - 1) * 1.5) * MaxThrottle

		local nratio = (math.min( Ratio, 1 ) + PitchAdd) / (1 + PitchAdd)

		local Volume = (Vol02 + Vol03 * nratio + (Vol02 * nratio + Vol03) * Throttle) * VolumeValue

		local invratio = math.max(1 - nratio,0)

		local start = nratio * LVS:GetFileLength( data.SoundFile )
		...
		self:lvsEmitSound( data.SoundFile, Volume, start - fadein, duration + fadein + fadeout, fadein, fadeout )

		EntTable.EngineSounds[ id ].NextUpdate = T + duration
	end
