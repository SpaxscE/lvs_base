...
function EFFECT:Render()
	...
	for i = 0, Num do
		local Frac = i / Num
		render.AddBeam( self.StartPos + Dir * Dist * Frac, (10 + 15 * ScaleMuzzle) * (1 - Frac), Frac, ColorScaled )
	end
	...
	render.DrawSprite( self.StartPos, 40, 40, ColorScaled )

	render.SetMaterial( mat4 )
	render.DrawBeam( self.StartPos, self.EndPos, 30 * TimeFracTracer, 0, (TimeFracTracer * Dist) / 512, ColorScaled )

	local SizeMuzzle = ScaleMuzzle * 40
	...
	render.DrawSprite( self.StartPos, SizeMuzzle, SizeMuzzle, color_white )
